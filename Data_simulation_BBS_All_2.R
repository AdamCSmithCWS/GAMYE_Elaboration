...
if(tp == "linear_2"){ #for linear trend sets all BETAs except 13 to 0
...
Beta_True[k,] <- yscale*0.75 + BETA_mid[k] + rnorm(nstrata,0,0.5)*xscale
  ...
  Beta_True[k,] <- rep(BETA_mid[k],nstrata)
  
}

if(tp == "linear_2" & k < nknots){ #for linear trend sets all BETAs except 13 to 0
