...
for(pp in c("t3")){
  for(prior_scale in c(4,10,15,20)){
    ...
    #STRATA_True <- log(2)
    output_dir <- "output/"
    out_base <- paste0(species_f,"_sim_Spatial_hier_with_YE_",tp,"_BBS")
   ...
   # if(!file.exists(paste0(output_dir,csv_files[1]))){
      ...
      mod.file = "models/GAM_Hier_Spatial_prior_sim_with_YE.stan"
...
save(file = "output/Hier_Spatial_with_YE_prior_sim_summary.RData",
     list = c("nsmooth_out",
              "NSMOOTH_out",
              "n_out",
              "N_out",
