...
data {
 ...
 int<lower=0> maxnsites_strata; //largest value of nsites_strata
  ...
  int<lower=1> nknots_year;  // nknots_year_smooth + 1
  int<lower=1> nknots_year_smooth;  // number of knots in the smoothing basis function for year
  int<lower=1> year_linear_col;  // column of the basis that is the linear component
...
parameters {
 ...
 vector[nstrata] strata_raw;
  ...
  real<lower=0> sdbeta_s[nknots_year_smooth];    // sd of GAM coefficients among strata 
  real<lower=0> sdbeta_1;    // sd of GAM coefficients among strata 
  ...
  real BETA_1; //fixed-effect linear trend
  vector[nknots_year_smooth] BETA_raw;//_raw; 
  matrix[nstrata,nknots_year_smooth] beta_raw;         // GAM strata level parameters
  vector[nstrata] beta_1_raw;
}

transformed parameters { 
  vector[ncounts] E;           // log_scale additive likelihood
  matrix[nstrata,nknots_year_smooth] beta_s;         // spatial effect slopes (0-centered deviation from continental mean slope B)
  vector[nstrata] beta_1;
  ...
  real <lower=0> sdbeta[nknots_year];
  matrix[nstrata,nyears] yeareffect;
  vector[nknots_year] BETA;
   
  sdbeta[1:nknots_year_smooth] = sdbeta_s;
  sdbeta[nknots_year] = sdbeta_1;
  
  BETA[1:nknots_year_smooth] = sdBETA*BETA_raw;
  BETA[year_linear_col] = BETA_1;

  for(k in 1:nknots_year_smooth){
    beta[,k] = (sdbeta[k] * beta_raw[,k]) + BETA[k];
  }
  beta[,year_linear_col] = (sdbeta[year_linear_col] * (beta_1_raw)) + BETA[year_linear_col];
...
model {
  ...
  sdbeta_s ~ student_t(3,0,1); // prior on sd of GAM parameters
  sdbeta_1 ~ student_t(3,0,0.5); // prior on sd of slopes
 ...
 for(s in 1:nstrata){
  ...
  BETA_1~ student_t(3,0,0.05);// prior on fixed effect hyper slope - trends > 10%/year are not common 
...
for(k in 1:nknots_year_smooth){
    ...
    beta_1_raw ~ icar_normal(nstrata, node1, node2);
