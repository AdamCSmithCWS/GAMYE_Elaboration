...
// intercepts and slopes
  ...
  real ste = sdste*ste_raw[site[i]]; // site intercepts
    ...
    E[i] =  smooth_pred[year[i],strat[i]] + strata + yeareffect[strat[i],year[i]] + eta*firstyr[i] + ste + obs + noise;
  }
...
model {
  sdnoise ~ student_t(3,0,0.5); //prior on scale of extra Poisson log-normal variance
  noise_raw ~ student_t(3,0,1);//student_t(nu,0,1); //normal tailed extra Poisson log-normal variance
   //noise_raw ~ normal(0,1);
  sdobs ~ student_t(3,0,0.5); //prior on sd of observer effects
