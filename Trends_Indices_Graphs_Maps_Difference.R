...
for(i in c(5:22)){#c(1:nrow(fls))){
  ...
  modl <- "Spatial"
      ...
      modl <- "Non-Spatial"
        ...
        ma <- MAs[i-(4+(length(MAs)*2))]
        load(paste0("Data/Simulated_data_",ma,"_breakpoint_cycle_BBS.RData"))
        realized_strata_map = strata_map 
        modl <- "Non-Hierarchical"
      }
    }
  }
...
# indices and trends ------------------------------------------------------
  ...
  # strat_inds_inds <- index_function(fit = stanfit,
  #                                     parameter = "ninds",
  #                                     year_1 = year_1,
  #                                     strat = st_n)
  
  if(!reald){
    true_inds <- log_true_traj %>% 
      mutate(True_scaled_index = exp(strata_True + YearEffect + Smooth)) %>% 
      ungroup() %>% 
    ...
    indices <- strat_inds$indices %>% 
      ...
      mutate(version = "full") %>% 
      left_join(.,obs_means) %>% 
      left_join(.,true_inds,by = c("true_year","Stratum_Factored"))
...
for(j in 1:length(tyrs)){
  ...
  tt <- trends_function(ind_list = strat_inds,
                        start_year = yy,
                        end_year = yy2) %>% 
    mutate(species = species,
           first_year = yy,
...
for(j in 1:length(tyrs)){
  ...
  TT <- trends_function(ind_list = sw_inds,
                        start_year = yy,
                        end_year = yy2) %>% 
    mutate(species = species,
           first_year = yy,
...
save(list = c("all_trends",
              ...
              "Ind_plots_list",
              "conv_summaries"),
     file = "output/Difference_real_data_summaries.RData")
