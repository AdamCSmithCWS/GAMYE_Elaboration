...
# loop through different mean intercepts ----------------------------------

balanced_raw <- balanced
strata_df_raw <- strata_df

for(STRATA_True in log(c(0.1,0.5,1,2,5))){
nstrata <- nrow(strata_df_raw)
...
## stratum-level smooths ----------------------------------------
  ...
  log_smooth_plot <- true_log_smooths %>% 
    ...
    left_join(.,strata_df_raw,by = c("Stratum_Factored")) %>% 
    arrange(Y,Year) 
  ...
  pdf(paste0("Figures/",species_f,"_",round(STRATA_True,2),"_True_smooth.pdf"),
      width = 11,
      height = 8.5)
  print(pfs)
  print(pf)
...
# Add observer, route intercepts ----------------------------
  ...
  balanced <- balanced_raw %>% left_join(.,log_true_traj,
...
save(list = to_save,
     file = paste0("Simulated_data_",species_f,"_",round(STRATA_True,2),"_BBS.RData"))
