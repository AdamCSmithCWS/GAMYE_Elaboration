...
Running a prior simulation for altnerative prior distributions on the SD of the GAM BETA parameters, the hyperparameters that control the shape and wiggliness of the survey-wide mean smooth. Because these priors and not intuitive, nor directly interpretable as biological processes or patterns, this simulation compares the effects of alternative priors on derived estimates of the long-term (53-year), and short-term (10-year) population trend estimates that would result from a population trajectory estimated with the same spline basis function used in this paper. We then compare the distribution of estimated trends to:

1.  the collection of realised trend estimates from a different statistical model applied to the North American Breeding Bird Survey ().

2.  our subjective prior knowledge about probable rates of change in wild bird populations at continental scales.

We compared three different prior distributions for the Standard Deviation parameter on the GAM parameters:

1.  gamma

2.  normal

3.  scaled t-distribution with 3 degrees of freedom
...
The gamma(2,0.5) and gamma(2,1) include some prior mass at ranges of variation in the trends that are not reasonable to expect in a natural population. They each include substantial prior mass where 90% of the 39-year population trends, vary by more than 30% for an overall stable population. E.g., they may include regional populations that have extreme declines (more than 15%/year) and regional populations that have similarly extreme increases. The gamma(2,2) prior suggest fewer of these extreme levels of variation, although it still includes a long-tail allowing for unlikely levels of variation.
