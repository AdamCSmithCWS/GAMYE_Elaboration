...
for(pp in c("t3")){#},"t4","t10")){
  for(prior_scale in c(0.5,1,2,3)){
    ...
    csv_files <- paste0(out_base,"-",1:3,".csv")
      ...
      mod.file = "models/GAM_Hier_prior_sim.stan"
...
save(file = "output/Hier_prior_sim_summary.RData",
     list = c("nsmooth_out",
              "NSMOOTH_out",
              "trends_out",
              "summ_out"))
...
load("output/Hier_prior_sim_summary.RData")
...
save(list = "trends_sd",
     file = "data/long_term_trend_hier_sd_prior_sim.RData")
...
ABSplot_long <- ggplot(data = TRENDS_abs,
  ...
  facet_wrap(vars(distribution),nrow = 3,ncol = 2)
