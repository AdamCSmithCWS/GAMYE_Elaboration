...
2.  Demonstrate the similarity of the alternative parameterisations of the hierarchical structure
...
3.  Fake data simulation to demonstrate accuracy
    ...
    1.  generate a balanced dataset with observations for all routes and all years, using the realised observer allocations.
        ...
        1.  smooth trajectory
            ...
            1.  set the mean BETAs

            2.  vary the betas using the neighbourhood relationships and a known sd_beta

            3.  generate the true trajectories for each stratum

                1.  plot them to confirm

            4.  generate the true trajectories for each route

            5.  add in the observer intercepts and stratum intercepts

            6.  add in the noise while generating the simulated counts

            7.  Confirm that the following parameters are accurately estimated:
        ...
        2.  Simple trajectory with same year-effects across all strata - shorebird structural demo

            1.  do same as above, but with hierarchical year-effects that have little variation among strata

        3.  linear trend - same as above, but with only one BETA != 0

        4.  breakpoint - same as above, but with only one (or two) BETA != 0
